...
mh_use_map AS (
    SELECT 'COMBINED' AS ivara_mh_use, 'CB' AS pioneers_code FROM dual UNION ALL
    SELECT 'FOUNDATION DRAIN', 'ZZ' FROM dual UNION ALL
    SELECT 'NOT APPLICABLE',   'ZZ' FROM dual UNION ALL
    SELECT 'SANITARY',         'SS' FROM dual UNION ALL
    SELECT 'STORM',            'SW' FROM dual
),
...
clean_longdesc AS (
  ...
  FROM (
    SELECT
      ...
      /* normalize longdescript into REAL HTML regardless of whether it arrives raw or entity-escaped */
      REPLACE(
        REPLACE(
          REPLACE(
            REPLACE(
              REPLACE(
                REPLACE(
                  REPLACE(
                    REPLACE(
                      REPLACE(
                        /* Step A: fix double-escaped entities first */
                        REPLACE(
                          REPLACE(
                            REPLACE(
                              REPLACE(
                      ...
                      /* Step B: decode core entities into characters */
                      '&lt;', '<'
                    ),
                    '&gt;', '>'
                  ),
...
macp_base AS (
    ...
    WHERE
        (e1.facilitytype IN (1,2) OR (e1.facilitytype = 10 AND ep2.unknfactype IN (2,3)))
),
...
pacp_base AS (
    ...
    FROM
        ...
        LEFT JOIN clean_longdesc cli ON cli.workordertaskoi = a.workordertaskoi
    WHERE
        e1.facilitytype = 8 OR (e1.facilitytype = 10 AND ep2.unknfactype IN (4,5,6,7,8))
),
...
lacp_base AS (
    ...
    FROM
        ...
        LEFT JOIN clean_longdesc cli ON cli.workordertaskoi = a.workordertaskoi
    WHERE
        e1.facilitytype = 4 OR (e1.facilitytype = 10 AND ep2.unknfactype IN (6,7))
),
...
macp_out AS (
    ...
    FROM
        ...
        LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_wall
          ON upper(trim(mat_wall.ivara_material)) =
             upper(trim(
               CASE
                 WHEN (b.facility_type = 2 OR (b.facility_type = 10 AND b.unknown_type IN (2,3)))
                 THEN NULL
        ...
        LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_bench
          ON upper(trim(mat_bench.ivara_material)) =
             upper(trim(
               CASE
                 WHEN (b.facility_type = 2 OR (b.facility_type = 10 AND b.unknown_type IN (2,3)))
                 THEN NULL
        ...
        LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_chan
          ON upper(trim(mat_chan.ivara_material)) =
             upper(trim(
               CASE
                 WHEN (b.facility_type = 2 OR (b.facility_type = 10 AND b.unknown_type IN (2,3)))
                 THEN NULL
