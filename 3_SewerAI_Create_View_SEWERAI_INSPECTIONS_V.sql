CREATE OR REPLACE FORCE EDITIONABLE VIEW "CUSTOMERDATA"."SEWERAI_INSPECTIONS_V" (
    ...
    WITH
/* ---------- Lookups used by MACP ---------- */ mh_use_map AS (
        SELECT
            'COMBINED' AS ivara_mh_use,
            'CB'       AS pioneers_code
        FROM
            dual
        UNION ALL
...
/* ---------- Shared CLOB-safe HTML cleaner (materialize once) ---------- */ clean_longdesc AS (
        SELECT /*+ MATERIALIZE */
            a.workordertaskoi,
            replace(
                regexp_replace(
...
/* ============================ MACP (Manhole & Catch Basin) ============================ */ macp_base AS (
        SELECT
            wo.wonumber
            || '.'
            ...
            WHEN e1.facilitytype = 10
                 AND ep2.unknfactype = 3 THEN
            'AMH'
            WHEN e1.facilitytype = 10
                 AND ep2.unknfactype = 2 THEN
            'ACB'
            ...
            CASE
            WHEN e1.facilitytype IN ( 1, 2 ) THEN
            'MACP'
            WHEN e1.facilitytype = 10
                 AND ep2.unknfactype IN ( 2, 3 ) THEN
            'MACP'
            ELSE
            NULL
    ...
    /* MACP-only reference (for CB / Unknown 2/3) */
            CASE
            WHEN ( e1.facilitytype = 2
                   OR ( e1.facilitytype = 10
                        AND ep2.unknfactype IN ( 2, 3 ) ) ) THEN
            e1.facilityid
            ELSE
            NULL
        ...
        WHERE
            ( e1.facilitytype IN ( 1, 2 )
              OR ( e1.facilitytype = 10
                   AND ep2.unknfactype IN ( 2, 3 ) ) )
    ),

/* ============================ PACP (Pipes) ============================ */ pacp_base AS (
        SELECT
            wo.wonumber
            || '.'
            || a.tasknumber                 AS work_orders_number,
            ...
            CASE
            WHEN e1.facilitytype = 8 THEN
            'PACP'
            WHEN e1.facilitytype = 10
                 AND ep2.unknfactype IN ( 4, 5, 6, 7, 8 ) THEN
            'PACP'
            ELSE
            NULL
            END                             AS inspection_type,
        ...
        WHERE
            e1.facilitytype = 8
            OR ( e1.facilitytype = 10
                 AND ep2.unknfactype IN ( 4, 5, 6, 7, 8 ) )
    ),

/* ============================ LACP (Service Connections) ============================ */ lacp_base AS (
        SELECT
            wo.wonumber
            || '.'
            || a.tasknumber                 AS work_orders_number,
        ...
        WHERE
            e1.facilitytype = 4
            OR ( e1.facilitytype = 10
                 AND ep2.unknfactype IN ( 6, 7 ) )
    ),

/* ============================ Normalize each branch (typed) ============================ */ macp_out AS (
        SELECT
            ...
            CAST(
                ...
                WHEN(b.facility_type = 2
                     OR(b.facility_type = 10
                        AND b.unknown_type = 2)) THEN
                sc_cb.pioneers_code
                ELSE
                sc_mh.pioneers_code
                END
            AS VARCHAR2(20))                                     AS cover_shape,
            CAST(
                CASE
                WHEN b.facility_type = 2
                     OR(b.facility_type = 10
                        AND b.unknown_type IN(2, 3)) THEN
                NULL
                ELSE
                mat_wall.pioneers_code
                END
            AS VARCHAR2(20))                                     AS wall_material,
            CAST(
                CASE
                WHEN b.facility_type = 2
                     OR(b.facility_type = 10
                ...
                NULL
                ELSE
                mat_bench.pioneers_code
                END
            AS VARCHAR2(20))                                     AS bench_material,
            CAST(
                CASE
                WHEN b.facility_type = 2
                     OR(b.facility_type = 10
                ...
                NULL
                ELSE
                mat_chan.pioneers_code
                END
            AS VARCHAR2(20))                                     AS channel_material,
            CAST(
                CASE
                WHEN(b.facility_type = 2
                     OR(b.facility_type = 10
            ...
            CAST(
                ...
                NULL
                ELSE
                b.mh_groundelevat_fl
                END
            AS NUMBER)                                           AS elevation,
            CAST(
                CASE
                WHEN(b.facility_type = 2
                     OR(b.facility_type = 10
                ...
                NULL
                END
            AS VARCHAR2(100))                                    AS frame_material,
            ...
            CAST(
                ...
                b.mh_neighbourhd
                END
            AS VARCHAR2(200))                                    AS drainage_area
        FROM
            macp_base                                      b
            LEFT JOIN customerdata.epsewerai_shape_code              sc_mh ON upper(
                TRIM(sc_mh.ivara_shape)
            ) = upper(
                TRIM(b.mh_shape)
            )
            LEFT JOIN customerdata.epsewerai_shape_code              sc_cb ON upper(
                TRIM(sc_cb.ivara_shape)
            ) = upper(
                TRIM(b.cb_shape)
            )
            LEFT JOIN (
            ...
            LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_wall ON upper(
                TRIM(mat_wall.ivara_material)
            ) = upper(
                TRIM(
                    CASE
                    WHEN(b.facility_type = 2
            ...
            LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_bench ON upper(
                TRIM(mat_bench.ivara_material)
            ) = upper(
                TRIM(
                    CASE
                    WHEN(b.facility_type = 2
            ...
            LEFT JOIN customerdata.epsewerai_wall_bench_channel_code mat_chan ON upper(
                TRIM(mat_chan.ivara_material)
            ) = upper(
                TRIM(
                    CASE
                    WHEN(b.facility_type = 2
        ...
        SELECT
            ...
            CAST(
                ...
                WHEN upper(
                    TRIM(b.raw_wwtype)
                ) IN('NOT APPLICABLE', 'N/A', 'NA') THEN
                'XX'
                ELSE
                NULL
                END
            AS VARCHAR2(50))                               AS pipe_use,
            CAST(coalesce(
                mc.pioneers_code, b.raw_material
            ) AS VARCHAR2(200))                            AS material,
            ...
            CAST(
                CASE
                WHEN b.facility_type = 10
                     AND b.unknown_type IN(4, 5) THEN
                b.facility_id
                ELSE
                b.base_pipe_ref
                END
            AS VARCHAR2(200))                              AS pipe_segment_reference,
            CAST(NULL AS VARCHAR2(200))                    AS lateral_segment_reference,
            ...
            CAST(b.raw_usinvertelev_fl AS NUMBER)          AS up_grade_to_invert,
            CAST(b.raw_dsgroundelev_fl AS NUMBER)          AS down_elevation,
            CAST(b.raw_dsinvertelev_fl AS NUMBER)          AS down_grade_to_invert,
        ...
        FROM
            pacp_base                            b
            LEFT JOIN customerdata.epsewerai_material_code mc ON upper(
                TRIM(mc.ivara_material)
            ) = upper(
                TRIM(b.raw_material)
            )
            LEFT JOIN customerdata.epsewerai_shape_code    sc ON upper(
                TRIM(sc.ivara_shape)
            ) = upper(
                TRIM(b.raw_shape)
            )
        ...
        SELECT
            ...
            AS VARCHAR2(50))                               AS pipe_use,
            CAST(coalesce(
                mc_sc.pioneers_code, b.lacp_pipetype
            ) AS VARCHAR2(200))                            AS material,
            CAST(NULL AS VARCHAR2(20))                     AS shape,
            CAST(NULL AS VARCHAR2(50))                     AS access_type,
            CAST(NULL AS VARCHAR2(200))                    AS manhole_number,
            ...
            CAST(
                CASE
                WHEN b.facility_type = 10
                     AND b.unknown_type IN(6, 7) THEN
                b.facility_id
                ELSE
                b.base_pipe_ref
                END
            AS VARCHAR2(200))                              AS lateral_segment_reference,
            CAST(NULL AS VARCHAR2(200))                    AS upstream_mh,
            CAST(NULL AS VARCHAR2(200))                    AS downstream_mh,
            CAST(b.work_order_task_uuid AS VARCHAR2(100))  AS work_order_task_uuid,
            CAST(b.dr_uuid AS VARCHAR2(100))               AS dr_uuid,
        ...
        FROM
            lacp_base                            b
            LEFT JOIN customerdata.epsewerai_material_code mc_sc ON upper(
                TRIM(mc_sc.ivara_material)
            ) = upper(
                TRIM(b.lacp_pipetype)
            )
    )
    SELECT
        "WORK_ORDERS_NUMBER",
